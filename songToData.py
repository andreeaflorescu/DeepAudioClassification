...
def createSpectrogram(filename,newFilename, path=spectrogramsPath):
	...
	command = "sox '/tmp/{}.mp3' -n spectrogram -Y 200 -X {} -m -r -o '{}.png'".format(newFilename,pixelPerSecond,path+newFilename)
	# print command
	p = Popen(command, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True, cwd=currentPath)
	output, errors = p.communicate()
	if errors:
...
def createSpectrogramsFromAudio():
	...
	for index,filename in enumerate(files):
		...
		spectogramPath = "{}{}.png".format(spectrogramsPath, newFilename)
		if not os.path.exists(spectogramPath):
			createSpectrogram(rawDataPath + filename, newFilename)
		# else:
		# 	print "Skip {} ; spectogram already exists".format(filename)
